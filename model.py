...
def get_balanced_classes(df: pd.DataFrame) -> pd.DataFrame:
...
class Model:
    ...
    def _prepare_shape_normalized(self, df: pd.DataFrame, shape_name: str) -> np.ndarray:
        ...
        return shape_arr.reshape(shape_arr.shape[0], shape_arr.shape[1], 1)
    ...
    def _train_shape_cnn_classifier(X: np.ndarray, y: np.ndarray) \
        ...
        model.add(layers.Conv2D(filters=32, kernel_size=(X.shape[1], kernel_size), strides=1,
    ...
    def run_shape_cnn_classifier(self, shape_name: str, encode: bool, c0_range_split: np.ndarray) -> None:
        """
...
if __name__ == '__main__':
    ...
    model.run_seq_classifier(k_list, c0_range_split, binary=False)
