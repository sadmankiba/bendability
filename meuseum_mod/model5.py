...
class nn_model:
    ...
    def create_model(self):
        ...
        first_layer_1 = ConvolutionLayer(
            filters=64, kernel_size=8, strides=1, data_format='channels_last', use_bias=True)
        
        
        fw_1 = first_layer_1(forward)
        rc_1 = first_layer_1(reverse)
        
        concat_1 = concatenate([fw_1, rc_1], axis=1)
        concat_2 = concatenate([rc_1, fw_1], axis=1)
        ...
        conv_2_1 = Conv1D(
            filters=32, kernel_size=8, strides=1, dilation_rate=1, data_format='channels_last', use_bias=True)
        conv_2_2 = Conv1D(
            filters=32, kernel_size=16, strides=1, dilation_rate=1, data_format='channels_last', use_bias=True)
        # conv_2_3 = Conv1D(
        #     filters=32, kernel_size=8, strides=1, dilation_rate=2, data_format='channels_last', use_bias=True)
        # conv_2_4 = Conv1D(
        ...
        arra_1 = concatenate([arra_1_1, arra_1_2])
        arra_2 = concatenate([arra_2_1, arra_2_2])
        ...
        if self.regularizer == 'L_1':
            outputs = Dense(1, kernel_initializer='normal', kernel_regularizer=regularizers.l1(
                0.001), activation=self.activation_type)(flat)
        elif self.regularizer == 'L_2':
            outputs = Dense(1, kernel_initializer='normal', kernel_regularizer=regularizers.l2(
                0.001), activation=self.activation_type)(flat)
        else:
