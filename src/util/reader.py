...
from util.util import roman_to_num, PathUtil
...
class DNASequenceReader:
    ...
    def read_yeast_genome(self, chr_num: YeastChrNum) -> pd.DataFrame:
        ...
        # Read file
        genome_file = open(
            f"{PathUtil.get_data_dir()}/input_data/yeast_genome/S288C_reference_sequence_R64-3-1_20210421.fsa"
        )
...
class GeneReader:
    def read_genes_of(self, chrm_num: YeastChrNum) -> pd.DataFrame:
        genes_file = f'{PathUtil.get_data_dir()}/input_data/gene/yeast_genes.tsv'
        rename_map = {
            'Gene.length': 'length',
    ...
    def read_transcription_regions_of(self, chrm_num: YeastChrNum) -> pd.DataFrame:
        ...
        """
        gene_utrs_file = f'{PathUtil.get_data_dir()}/input_data/gene/yeast_gene_utrs.tsv'
        rename_map = {
