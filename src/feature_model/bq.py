...
from util.util import gen_random_sequences, sorted_split, cut_sequence, PathUtil
...
def find_bq(df: pd.DataFrame, unit_len: int, df_str: str):
    ...
    seq_occur_map = occurence.normalize_bin_occurence(seq_occur_map,
                                                      len(sorted_dfs[0]))
...
def plot_bq():
    ...
    if len(sorted_unit_seq_bq_pair) > 20:
        pairs_to_show = [
            pair for (i, pair) in enumerate(sorted_unit_seq_bq_pair)
    ...
    plt.savefig(f'{PathUtil.get_figure_dir()}/cnl_bq_5.png')
...
def plot_indirect_shape_seq_bq():
    ...
    for df_name, df in [('cnl', cnl_df), ('rl', rl_df)]:
        for unit_bp_size in [8, 9]:
            seq_bq_map = find_bq(df, unit_bp_size,
                                 f'{df_name}_{seq_start_pos}_{seq_end_pos}')

            sorted_unit_seq_bq_pair = sorted(seq_bq_map.items(),
                                             key=lambda x: x[1])
            print('sorted_unit_seq_bq_pair - no. of keys',
            ...
            for shape_name in ['HelT', 'MGW', 'ProT', "Roll"]:
                ...
                for i in range(shape_arr.shape[0]):
                    ...
                    plt.plot(range(shape_arr.shape[1]),
                             shape_arr[i],
                             linestyle='-',
                             marker='o',
                             color=color)
                ...
                blue_patch = mpatches.Patch(
                    color='r',
                    label=
                    f'Top {TOP_N} {shape_arr.shape[1]}-shape sequences associated with high loopability'
                )
                green_patch = mpatches.Patch(
                    color='b',
                    label=
                    f'Top {TOP_N} {shape_arr.shape[1]}-shape sequences associated with low loopability'
                )
                ax = plt.gca()
                ...
                plt.savefig(
                    f'{PathUtil.get_figure_dir()}/shape_seq/{df_name}_{seq_start_pos}_{seq_end_pos}_{shape_name}_{shape_arr.shape[1]}_{TOP_N}_seq_impact.png'
                )
