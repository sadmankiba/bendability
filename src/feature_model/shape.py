    ...
    with open(file_path, "r") as file:
        ...
        for data in file.readlines():
            ...
            if data[0] == '>':
                ...
                shape_float_list = [
                    None if val == 'NA' else float(val)
                    for val in shape_str_list
                ]
...
def find_all_shape_values():
    ...
    def get_helt_roll_df(
        ...
        six_df = pd.concat(
            [six_seq_df,
             pd.DataFrame({shape_str: six_arr[:, 2].flatten()})],
            axis=1)
