...
class Genes:
    ...
    def plot_mean_c0_vs_dist_from_dyad(self) -> Path:
        ...
        mean_c0 = (frwrd_mean_c0 * len(frwrd_p1_dyads) + 
                    rvrs_mean_c0 * len(rvrs_p1_dyads)) / len(self._tr_df)
    ...
    def in_promoter(self, bps: np.ndarray | list[int] | pd.Series) -> np.ndarray:
        ...
        """
        frwrd_prmtr_rgn = self._frwrd_tr_df()['start']\
                        .apply(lambda bp: np.arange(bp - 400, bp + 400 + 1))
        rvrs_prmtr_rgn = self._rvrs_tr_df()['end']\
                        .apply(lambda bp: np.arange(bp - 400, bp + 400 + 1))

        prmtr_rgn = np.concatenate((np.array(frwrd_prmtr_rgn.tolist()).flatten(), 
