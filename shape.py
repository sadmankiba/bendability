...
def find_all_shape_values():
    ...
    def get_helt_roll_df(five_arr, six_arr, shape_str: str, ):
        ...
        imbp_suffixed_five_seq_df = pd.DataFrame(
            {'Sequence': [ imaginary_bp + seq for seq in five_seq_df['Sequence'].tolist() ]}
        )
        imbp_prefixed_five_seq_df = pd.DataFrame(
            {'Sequence': [ seq + imaginary_bp  for seq in five_seq_df['Sequence'].tolist() ]}
        )
    
        suf_df = pd.concat([imbp_suffixed_five_seq_df, pd.DataFrame({shape_str: five_arr[:,1].flatten()})], axis=1)
        pref_df = pd.concat([imbp_prefixed_five_seq_df, pd.DataFrame({shape_str: five_arr[:,2].flatten()})], axis=1)
        six_df = pd.concat([six_seq_df, pd.DataFrame({shape_str: six_arr[:,2].flatten()})], axis=1)
